...
//    }
//
//    public void setCacheStateful(HashMap<RunStateMap, MultiBiMap<Long, Long>> cacheStateful) {
...
//    
    ...
    public boolean checkStateType(GraphEvent e) throws Exception{
        ...
        if(s.getStateType().equalsIgnoreCase("kleeneClosure") && s.getKleeneCounter()>=1 && currentState != this.nfa.getSize() - 1 || (
        		s.isManager() && s.getSubstatesType()==2 ) ){
            ///then check first if the stateevent counter is >=1, if so then check the event on next state predicate if so, then extend it next two states and add a new run with it
            
            
            //  state[currentState] = 2;
            ///check the next state predicate///considering that the next state is a normal one
     ...
     */
    public void addGraphEvent(GraphEvent e){//1 incomplete, 2 complete ,3 kleene closure      //Comment: For the first RunOptimised If state type is normal then make
        //the state complete and move to the next state
        //String stateType = this.nfa.getStates()[currentState].getStateType();
        if(this.nfa.getStates()[currentState].getStateType().equalsIgnoreCase("normal") || this.nfa.getStates()[currentState].getStateType().equalsIgnoreCase("negation") ||this.nfa.getStates()[currentState].getStateType().equalsIgnoreCase("optional") ){
        ...
        if(this.count == 1){
           // this.startTimeStamp = e.getTimeStamp();
            ...
            //	this.partitonId = e.getAttributeByName(this.nfa.getPartitionAttribute()); ///Change it
            //}
       ...
       public void addGraphEvent(GraphEvent e,long id){//1 incomplete, 2 complete ,3 kleene closure      //Comment: For the first RunOptimised If state type is normal then make
        //the state complete and move to the next state
        //String stateType = this.nfa.getStates()[currentState].getStateType();
        if(this.nfa.getStates()[currentState].getStateType().equalsIgnoreCase("normal") || this.nfa.getStates()[currentState].getStateType().equalsIgnoreCase("negation") ){
            // this.eventIds.add(e.getId());
            state[currentState] = 2;
            this.count ++;
            if(currentState == this.nfa.getSize() - 1){
    ...
    */
    ...
    public Object clone(int id) throws CloneNotSupportedException{
        ...
        o.setNfa(nfa);
       // o.setCache(( HashMap<Integer, MultiBiMap<SP, OP> > )this.getCache().clone());
...
//    }
  ...
  /*  public HashMap<Integer, MultiBiMap<SP, OP>> getCache() {
        return cache;
    }
