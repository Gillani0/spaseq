...
public class Filter
{
    ...
    /*   public boolean filterValueManipulation(String filter) throws ParseException, DatatypeConfigurationException {
    filter = filter.replace("'", "");
    
    if (filter.contains("<") && filter.contains(">")) {    //// A proper pattern May be?
    filter = filter.replace("<", "");
    filter = filter.replace(">", "");
    
    // /remove < and >
    this.filterValue       = factory.createURI(filter);
    this.filterMappedValue = this.dictImpl.addResource((Resource) this.filterValue);
    
    return true;
    } else {
    if (isInteger(filter)) {
    this.filterValue       = factory.createLiteral(Integer.parseInt(filter));           //  0: int, 1: float, 2: decimal, 3: DataTime, 4: Double, 5: string, 6: Long, 7: georgian Calender,  8: Boolean
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    this.filterDataType    = 0;
    return true;
    } else if (isDateTime(filter)) {
    Date datetime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(filter);
    
    this.filterValue       = factory.createLiteral(datetime);
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    this.filterDataType   = 3;
    return true;
    } else if (isFloat(filter)) {
    this.filterValue       = factory.createLiteral(Float.parseFloat(filter));
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    ...
    if (isDouble(filter)) {
    this.filterValue       = factory.createLiteral(Double.parseDouble(filter));
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    ...
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    this.filterDataType   = 6;
    return true;
    } else if (isBoolean(filter)) {
    this.filterValue       = factory.createLiteral(Boolean.parseBoolean(filter));
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    this.filterDataType   = 8;
    return true;
    } else if (isXMLGregorianCalender(filter)) {
    this.filterValue       =
    factory.createLiteral(DatatypeFactory.newInstance().newXMLGregorianCalendar(filter));
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    this.filterDataType   = 7;
    return true;
    } else if (filter instanceof String) {
    this.filterValue       = factory.createLiteral(filter.toString());
    this.filterMappedValue = this.dictImpl.addLiteral((Literal) this.filterValue);
    this.filterDataType    = 5;
    return true;
    }
    }
    ...
    */
    ...
    public boolean filterValuePersistant(String s) throws ParseException, DatatypeConfigurationException {
        ...
        if (s.contains("<") && s.contains(">")) {    //// A proper pattern May be?
            ...
            return true;
        } else if(s.contains("^^")) {  //if contains the data type then use the datatype, otherwise consider it as a string
            String[] ss= s.split("\\^");
           ...
           // nL.setType(4);
        	...
        	try{
              ...
              this.filterValue=nL;
        		...
        		System.out.println("Unable to parse the Filter caluse in the query, please check if the data type is given such as 'Filter(?x > '2'^^xsd:integer)'");
        	}
  ...
  /// // 0: int, 1: float, 2: decimal, 3: DataTime, 4: Double, 5: string, 6: Long, 7:boolean
    ...
    public void setFilterValue(Object filterValue) throws ParseException, DatatypeConfigurationException {
        this.filterValuePersistant(filterValue.toString());
        
        // this.filterValue = filterValue;
    }
    
    /* public String getFilterComp() {
    return filterComp;
    }
    
    public void setFilterComp(String filterComp) {
