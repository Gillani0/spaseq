...
import edu.telecomstet.cep.rulesmodel.UnionPatternData;
...
//             + "   ?post sib:hashtag ?tag2. } "+
   ...
   System.out.println(tree.toStringTree(parser));
    ...
    ListenerQueryContextUpdated ListenerQuery=new ListenerQueryContextUpdated(dict);
...
//        result.stream().sorted((e1,e2)-> Integer.compare(e1., y));
       ...
       result= result .stream().sorted((e1, e2) -> Integer.compare(e1.getId(),
                e2.getId())).collect(Collectors.toList());
       
       
       ///We get the sorted list of elements, now we need to add pattern data stuff in the NFA data with an additional automata
        //go through the whole list of pattern data and compare it with the NFA and make a new list out of it.
...
//    		   NFAData first= test.stream().filter(x->x.getStateType().equals(d.getPattName())).findFirst().get();
