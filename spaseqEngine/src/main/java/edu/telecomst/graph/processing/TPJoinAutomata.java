...
public class TPJoinAutomata {
   ...
   // private int proceed=0;
    ...
    public boolean run(Automata automata, DictionaryOpImpl dic,Map<RunStateMap, MultiBidirectionalIndex> c,Map<KBindex, MultiBiMap<Long,SP>> kbcache,int r,int st){
        // if(proceed==0){
        
      
        
        if( automata.getStates().stream().filter(x->x.getRule().getC2().size()>0).count()==automata.getStates().size()){
            //    proceed=1;
    ...
    public boolean tpJoinStart(Automata automata, DictionaryOpImpl dic,Map<RunStateMap,MultiBidirectionalIndex> c,Map<KBindex, MultiBiMap<Long,SP>> kbcache,int r,int st){
        ...
        /**
         * Check if there exists am external KB that is required to be joined with the
         * current event
         */
        ...
        /**
         * Extract the results from the joined tables using the incrementally produced indexes
         */
     ...
     ///if the resulted 
      for(int i=1;i<automata.getStates().size();i++){
           ...
           if(!automata.getStates().get(i).getEdge().getRreal().getrIndex().isEmpty() && automata.getStates().get(i).getEdge().getRreal().getrIndex().keySize()<=min ){
        ...
        /*
         * If there is just one triple pattern, then put all the things from C in the stateful cache
         */
        if(keys ==null && automata.getStates().size() ==1){
        	 ...
        	 while(it.hasNext()){
				...
				Map.Entry pair = (Map.Entry)it.next();
        		 result.getrIndex().put(aS.getRule().getPredicate().getMappedValue(), new SP((Long)pair.getKey(),(Long)pair.getValue()));
        	 }
...
private void stateFulJoin(AutomataState currState, Automata a, DictionaryOpImpl dic,Map<RunStateMap, MultiBidirectionalIndex> c,int r){
    
  for(StateDependability sd:currState.getRule().getS_depends()){
	  
	  
	  ///TODO: check if the state dependability is equal to the normal dependability
	  ...
	  if(sd.getD_On()==1 && sd.getD_part()==1){
         ...
         // System.out.println("Dependqbility ID; "+ sd.getIdNFA());
          ...
          if(!currState.getEdge().getRreal().getR().isEmpty()){
        	  this.ssJoinOptimisedForRRStateful(statef.getR(), currState.getEdge().getRreal(),currState.getEdge().getrCopied() );
        	  ...
        	  this.ssJoinOptimisedForRRStateful(statef.getR(), currState.getEdge().getrCopied(),currState.getEdge().getRreal() );
        	  ...
        	  this.ssJoinOptimisedForCCStateful(statef.getR(), currState.getRule().getC2(), currState.getEdge().getRreal());  
...
///Change the function for multiple filter clause and for stateful join there could be only 1 triple patter
///Plus used the older index value and do not update the index value even after the join
...
private long dependabilityComparison(List<Dependability> dd, StateDependability sd , int id){
	...
	return dd.stream().filter(x->x.getDependability_On()==sd.getD_On() && x.getDependability_part()==sd.getD_part() && x.getDependabilty_id() < id).count();
	
	
}


private void stateFulFilterJoin(AutomataState currState,  DictionaryOpImpl dic,Map<RunStateMap,MultiBidirectionalIndex> c,int r){
	...
	for(Filter filter:currState.getRule().getFilter()){
        ...
        if(currState.getRule().getFilterType()==2 && filter.getStateFulness()==1){
            ...
            ///   System.out.println(filter.getD().getD_id());
               ///use the dd to put the value trple by triple instead of states
        	
               MultiBiMap<Long,Long> stateful= c.get(new RunStateMap(filter.getD().getIdNFA(),r,filter.getD().getD_id())).getR();
         ...
         //   if(!s.getEdge().getR().isEmpty()){
                switch (currState.getRule().getFilterType1Operator()) {
                    case "=":{
                        ...
                        if(!currState.getEdge().getRreal().getR().isEmpty()){
                       this.ooHashJoinCCStateful(stateful, currState.getEdge().getRreal().getR(), currState.getEdge().getrCopied());
                       ///TODO: change the cc join and check every thing, it could be 
                       if(checker){
                          currState.getEdge().getRreal().getR().clear();
                          currState.getEdge().getRreal().getrIndex().clear();
                       }
                           ...
                           this.ooHashJoinCCStateful(stateful, currState.getEdge().getrCopied().getR(), currState.getEdge().getRreal());
                    ...
                    case "!=":{
                      //  ooNonEqualJoin(s.getEdge().getR(), currState.getRule().getC(), currState.getEdge().getRCopied(),s.getRule().getPredicate().getMappedValue());
                          ...
                          if(!currState.getEdge().getRreal().getR().isEmpty()){
                      this.ooNonequiHashJoin(stateful, currState.getEdge().getRreal().getR(), currState.getEdge().getrCopied());
                      ...
                      if(checker){
                        ...
                        this.ooNonequiHashJoin(stateful, currState.getEdge().getrCopied().getR(), currState.getEdge().getRreal());
                 ...
                 //   this.complexFilterJoin(stateful, currState.getEdge().getRreal().getR(), currState.getEdge().getrCopied(), dic, currState.getRule().getFilterType1Operator(), currState.getStateDataType());
                    complexFilterforR(stateful,currState.getEdge().getRreal(),currState.getEdge().getrCopied(), dic, currState.getRule().getFilterType1Operator(), currState.getStateDataType());
                    ...
                    if(checker){
                       ...
                       // this.complexFilterJoin(stateful, currState.getEdge().getrCopied().getR(), currState.getEdge().getRreal(), dic, currState.getRule().getFilterType1Operator(), currState.getStateDataType());  
                   
                        complexFilterforR(stateful,currState.getEdge().getrCopied(),currState.getEdge().getRreal(), dic, currState.getRule().getFilterType1Operator(), currState.getStateDataType());
                    ...
                    } else if(currState.getEdge().getRreal().getR().isEmpty()&&currState.getEdge().getrCopied().getR().isEmpty() && !currState.getRule().getC2().isEmpty()){
                    	///new Complex Filter Statement
                    	 this.complexFilterforC(stateful,currState.getRule().getC2(), currState.getEdge().getRreal(), dic, currState.getRule().getFilterType1Operator(), currState.getStateDataType());
                    }
                    
                    
                   
                    
                    //TODO: add one more IF caluse for C if both Rreal and Rcopied are empty
                    }break;
     ...
     private void tripleJoinStage1(AutomataState currState, Automata a, DictionaryOpImpl dic,Map<RunStateMap,MultiBidirectionalIndex> c,int r){
       
     //   stateFulJoin(AutomataState currState, Automata a, DictionaryOpImpl dic,HashMap<RunStateMap, MultiBiMap<Long, Long>> c,int r);
                ...
                //  resultHandler=true;
                 ...
                 for(AutomataState s:a.getStates()){ ///WTF
                     if(s.getStateId()==dd.getDependabilty_id()){
                         ...
                         if(!s.getRule().getDepends().isEmpty() &&   s.getRule().getDepends().get(0).getDependabilty_id()==currState.getStateId()){
                          s.getRule().getDepends().get(0).setJoined(1);
                         }else if(!s.getRule().getDepends().isEmpty() && s.getRule().getDepends().size()>1 &&  s.getRule().getDepends().get(0).getDependabilty_id()==currState.getStateId()){
                 ...
                 if(dd.getDependability_On()==1 && dd.getDependability_part()==1 && stop==0){
                   ...
                   //  incrementalOSJoin(currState,sJoin,a);
                     ...
                     this.incrementalSOJoinStage2(currState, sJoin);
                         ...
                         dd.getDependability_On()==0 && dd.getDependability_part()==0  && stop ==0){
...
//       
                ...
                if(joinState.getEdge().getRreal().getR().isEmpty() && joinState.getEdge().getrCopied().getR().isEmpty()){ /// the use the both C and put it in Rreal of both
                    ...
                    if(currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
                        ...
                        this.ssJoinOptimisedForCC(currState.getRule().getC2(), joinState.getRule().getC2(), currState.getEdge().getRreal(), joinState.getEdge().getRreal());
                              ...
                              this.ssJoinOptimisedForCC(joinState.getRule().getC2(),currState.getRule().getC2(),  joinState.getEdge().getRreal(), currState.getEdge().getRreal());
                        }
     ...
     //      this.ssJoinOptimisedForRC(currState.getRule().getC2(), joinState.getEdge().getrCopied(), joinState.getEdge().getRreal(), currState.getEdge().getRreal());
      ...
      //// check the tablesomappedcopied
        ...
        private void ssJoinOptimisedForCCStateful( MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r2){
        ...
        private void ssJoinOptimisedForRRStateful( MultiBiMap<Long, Long> _c1,MultiBidirectionalIndex _c2,MultiBidirectionalIndex  _r2){
            ///thats the issues CC statefull all the time, again two conditions
   ...
   if(joinState.getEdge().getRreal().getR().isEmpty() && joinState.getEdge().getrCopied().getR().isEmpty()){ /// the use the both C and put it in Rreal of both
                 ...
                 ///reverse function
                        ...
                        this.osHashJoinRC(joinState.getRule().getC2(), currState.getEdge().getRreal(), joinState.getEdge().getRreal(),currState.getEdge().getrCopied());
                         if(checker){
                     ...
                     //reverse function
                        this.osHashJoinRC(joinState.getRule().getC2(), currState.getEdge().getrCopied(), joinState.getEdge().getRreal(),currState.getEdge().getRreal());
           ...
           //use Real 
           if(currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty()){
                this.soHashJoinRC(currState.getRule().getC2(), joinState.getEdge().getRreal(), currState.getEdge().getRreal(), joinState.getEdge().getrCopied());
               ...
               joinState.getEdge().getRreal().getrIndex().clear();
                    ...
                    if(!currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
                        ...
                        soHashJoinRC2(currState.getEdge().getRreal(),joinState.getEdge().getRreal(), currState.getEdge().getrCopied(),joinState.getEdge().getrCopied());
                               ...
                               osHashJoinRC2(joinState.getEdge().getRreal(),currState.getEdge().getRreal(),joinState.getEdge().getrCopied(), currState.getEdge().getrCopied());
                        }
                    ...
                    if(currState.getEdge().getrCopied().getR().size()<joinState.getEdge().getRreal().getR().size()){
                        soHashJoinRC2(currState.getEdge().getrCopied(),joinState.getEdge().getRreal(), currState.getEdge().getRreal(),joinState.getEdge().getrCopied());
                          ...
                          osHashJoinRC2(joinState.getEdge().getRreal(),currState.getEdge().getrCopied(), joinState.getEdge().getrCopied(),currState.getEdge().getRreal());
                    }
       ...
       //    if(checker){
         //      joinState.getEdge().getRreal().getR().clear();
          ...
          // }
           ...
           if(currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty()){
                this.soHashJoinRC(currState.getRule().getC2(), joinState.getEdge().getrCopied(), currState.getEdge().getRreal(), joinState.getEdge().getRreal()); 
           ...
           if(checker){
            ...
            if(!currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
             ...
             if(currState.getEdge().getRreal().getR().size()<joinState.getEdge().getrCopied().getR().size()){
                  ...
                  osHashJoinRC2(joinState.getEdge().getrCopied(),currState.getEdge().getRreal(),joinState.getEdge().getRreal(), currState.getEdge().getrCopied());
             }
                     ...
                     currState.getEdge().getRreal().getR().clear();
                      currState.getEdge().getRreal().getrIndex().clear();
                       ...
                       if(currState.getEdge().getrCopied().getR().size()<joinState.getEdge().getrCopied().getR().size()){
                             ...
                             osHashJoinRC2(joinState.getEdge().getrCopied(),currState.getEdge().getrCopied(), currState.getEdge().getRreal(),joinState.getEdge().getRreal());
                       }
    ...
    ///////////////////////######################################################################////////////////////////////////////////
      private void incrementalOSJoinStage2(AutomataState currState, AutomataState joinState){//AutomataState currState, AutomataState joinState, Automata a, long pred1, long pred2){
   ...
   if(joinState.getEdge().getRreal().getR().isEmpty() && joinState.getEdge().getrCopied().getR().isEmpty()){ /// the use the both C and put it in Rreal of both
                    ...
                    if(!currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
                     // this.osHashJoinRC(null, null, null, null);
                      ...
                      this.soHashJoinRC(joinState.getRule().getC2(), currState.getEdge().getRreal(), joinState.getEdge().getRreal(),currState.getEdge().getrCopied());
                     ...
                     currState.getEdge().getRreal().getR().clear();
                      currState.getEdge().getRreal().getrIndex().clear();
                        ...
                        this.soHashJoinRC(joinState.getRule().getC2(), currState.getEdge().getrCopied(), joinState.getEdge().getRreal(),currState.getEdge().getRreal());
           ...
           //use Real 
           if(currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty()){
                this.osHashJoinRC(currState.getRule().getC2(), joinState.getEdge().getRreal(),currState.getEdge().getRreal(), joinState.getEdge().getrCopied());
            ...
            if(checker){
               ...
               joinState.getEdge().getRreal().getrIndex().clear();
                    ...
                    if(!currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
                        ...
                        if(currState.getEdge().getRreal().getR().size()<joinState.getEdge().getRreal().getR().size()){
                              osHashJoinRC2(currState.getEdge().getRreal(),joinState.getEdge().getRreal(),currState.getEdge().getrCopied(),joinState.getEdge().getrCopied());
                        }else{
                     ...
                     currState.getEdge().getRreal().getR().clear();
                      ...
                      if(currState.getEdge().getrCopied().getR().size()<joinState.getEdge().getRreal().getR().size()){
                            osHashJoinRC2( currState.getEdge().getrCopied(),joinState.getEdge().getRreal(),currState.getEdge().getRreal(), joinState.getEdge().getrCopied());
                      }else{
       ...
       //    if(checker){
         //      joinState.getEdge().getRreal().getR().clear();
          ...
          // }
           ...
           if(currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty()){
                this.osHashJoinRC(currState.getRule().getC2(), joinState.getEdge().getrCopied(),  currState.getEdge().getRreal(),joinState.getEdge().getRreal()); 
            ...
            if(!currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
             ...
             //   osHashJoinRC2(currState.getEdge().getRreal(),joinState.getEdge().getrCopied() , currState.getEdge().getrCopied(),joinState.getEdge().getRreal());
                      ...
                      currState.getEdge().getRreal().getrIndex().clear();
                        ...
                        if(currState.getEdge().getrCopied().getR().size()<joinState.getEdge().getrCopied().getR().size()){
                              osHashJoinRC2(currState.getEdge().getrCopied(),joinState.getEdge().getrCopied(), currState.getEdge().getRreal(),joinState.getEdge().getRreal());
                        }else{
   ...
   if(joinState.getEdge().getRreal().getR().isEmpty() && joinState.getEdge().getrCopied().getR().isEmpty()){ /// the use the both C and put it in Rreal of both
               ...
               joinState.getEdge().getRreal().getrIndex().clear();
                    ...
                    if(!currState.getEdge().getRreal().getR().isEmpty() && currState.getEdge().getrCopied().getR().isEmpty() ){
                        ...
                        ooHashJoinRC2(joinState.getEdge().getRreal(), currState.getEdge().getRreal(), joinState.getEdge().getrCopied(),currState.getEdge().getrCopied());
                              ...
                              ooHashJoinRC2( currState.getEdge().getRreal(),joinState.getEdge().getRreal(), joinState.getEdge().getrCopied(),currState.getEdge().getrCopied());
                        }
      ...
      private void osHashJoinCC(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex _r1,MultiBidirectionalIndex  _r2){
           ...
           private void osHashJoinCCStateful(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r2){
    ...
    ///o for current state   
       private void soHashJoinCC(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex _r1,MultiBidirectionalIndex  _r2){
            ...
            for(Long o:_c1.values()){//change the order
             ...
             private void soHashJoinCCStateful(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r2){
     ...
     private void ooHashJoinCCStateful(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r2){
       ...
       private void complexFilterforR(MultiBiMap<Long, Long> _c1,MultiBidirectionalIndex  _r2,MultiBidirectionalIndex  _r3,DictionaryOpImpl dic,String oper, int dataType){
    	   //first entry is the statefull map
    	   for(Long op:_c1.values()){
    		   for(Long op2:_r2.getR().values()){
    			   
    			   
    			   if(this.compLiteral.evaluateEventType(dic.getLiteralAdaptive(op),dic.getLiteralAdaptive(op2),oper, dataType, 1)){
                       checker=true;
       ...
       private void complexFilterforC(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r3,DictionaryOpImpl dic,String oper, int dataType){
    	   for(Long op:_c1.values()){
       ...
       private void complexFilterJoin(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r2,DictionaryOpImpl dic,String oper, int dataType){
       ...
       private void ooNonequiHashJoin(MultiBiMap<Long, Long> _c1,MultiBiMap<Long, Long> _c2,MultiBidirectionalIndex  _r2){
