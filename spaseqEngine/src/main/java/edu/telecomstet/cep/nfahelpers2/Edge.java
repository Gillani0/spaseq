...
public final class Edge {
    ...
    //   private  ArrayList<Rule> dependencyRule;
      // private final  NFAData nfaData;
        //private  ContinuousSPARQLEngineController spE = new  ContinuousSPARQLEngineController();
...
public Edge(NFAData nfaData, int edgeTypeNum){
 ...
 public boolean evaluatePredicateandStatQuery (GraphEvent event ,State s,Map<RunStateMap, MultiBidirectionalIndex> cache,Map<KBindex, MultiBiMap<Long,SP>> kbcache, int r) throws Exception 
        {
            /**
             * Send Event to the Query Controller and If it returns some result then return
     ...
     // return new GraphQueryAdapter().sparqlAdapter(event,s.getNfaData(),dependencyRule,spE,cache,r,s.getOrder()) ;  //send the run id and state orderas well
        return this._gp.run(s.getNfaData().getAutomta(),event,s.getNfaData().getDicionary(), cache,kbcache, r,s.getOrder());
       // return true;
   ...
   public boolean evaluateGraphEvent(GraphEvent currentEvent, RunOptimised r,State s,Map<RunStateMap,MultiBidirectionalIndex > cache,Map<KBindex, MultiBiMap<Long,SP>> kbcache) throws Exception { 
         
        
          return this._gp.run(s.getNfaData().getAutomta(),currentEvent,s.getNfaData().getDicionary(), cache,kbcache, r.getId(),s.getNfaData().getOrder());
